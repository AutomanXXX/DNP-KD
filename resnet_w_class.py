...
class ConvNeXt_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pwconv2(x)
        x = self.gamma * x
        x_emb = x.permute(0, 3, 1, 2) # (N, H, W, C) -> (N, C, H, W)

        return x_emb.contiguous()
    
class C_Block(nn.Module):
    def __init__(self, dim, num_classes=0, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
...
class simple_add_Block(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((1,dim,1,1)), requires_grad=True) if layer_scale_init_value > 0 else None
...
class A_2D_Block(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((1,dim,8,8)), requires_grad=True) if layer_scale_init_value > 0 else None
        self.Sigmoid = nn.Sequential(nn.Sigmoid())
    def forward(self, x):
        # print(self.gamma.shape)
        # print(x.shape)
        x_emb = self.Sigmoid(self.gamma) * x

        return x_emb.contiguous(), self.Sigmoid(self.gamma) 

class A_2D_linear_Block(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((1,dim,8,8)), requires_grad=True) if layer_scale_init_value > 0 else None
        self.Sigmoid = nn.Sequential(nn.Sigmoid())
...
class simple_linear_conv1(nn.Module):
    def __init__(self, dim):
        super().__init__()

        self.pwconv1 = nn.Conv2d(dim, 4 * dim, kernel_size=1, stride=1, padding=0)
        self.pwconv2 = nn.Conv2d(4 * dim, dim, kernel_size=1, stride=1, padding=0)

    def forward(self, x):

        x = self.pwconv1(x)
        x_emb = self.pwconv2(x)

        return x_emb.contiguous()   

class layernorm_bk(nn.Module):
    def __init__(self, dim):
        super().__init__()
...
class convnext_conv1(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
...
class convnext_conv1_conv3(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv2d(4 *dim, 4 *dim, kernel_size=3, padding=1, groups=dim)
        self.norm = LayerNorm(dim, eps=1e-6,data_format="channels_first")
        self.norm_2 = LayerNorm(4 *dim, eps=1e-6,data_format="channels_first")
        self.act = nn.GELU()
        self.pwconv1 = nn.Conv2d(dim, 4 * dim, kernel_size=1, stride=1, padding=0)
        self.pwconv2 = nn.Conv2d(4 * dim, dim, kernel_size=1, stride=1, padding=0)        
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((1,dim,1,1)), requires_grad=True)
